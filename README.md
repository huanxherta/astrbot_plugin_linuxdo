...
这是一个为 [AstrBot](https://github.com/AstrBotDevs/AstrBot) 框架开发的插件，旨在方便用户直接在聊天机器人中获取 [LINUX DO](https://linux.do) 社区的热帖、活跃讨论及最新话题。

## 🌟 功能特性

- **今日热榜**: 获取社区每日热门话题列表 (`/ld_top`)。
- **活跃讨论**: 获取社区最近有新动态的话题 (`/ld_new`)。
- **最新发布**: 专门获取社区**刚发布**的新贴，不含旧帖回复 (`/ld_fresh`)。
- **智能过滤**: 自动过滤置顶帖，只看新鲜内容。
- **清爽 UI**: 使用**合并转发消息**展示结果，点开即看，不刷屏。
- **WebUI 配置**: 支持在管理后台直接调整各指令展示数量和过滤开关。
...
| `/ld_new` | 获取最近活跃讨论 (含回复) | `/ld_new` |
| `/ld_fresh` | 获取最新发布的话题 (纯新帖) | `/ld_fresh` |
