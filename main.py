...
@register("linuxdo", "GeminiCLI", "LINUX DO 社区助手", "1.7.0")
class LinuxDoPlugin(Star):
    ...
    async def get_latest_activity(self, event: AstrMessageEvent):
        '''获取 LINUX DO 最近活跃话题'''
        yield event.plain_result("✨ 正在拉取大批量活跃讨论...")
    ...
    async def get_time_topics(self, event: AstrMessageEvent):
        '''获取 LINUX DO 最新发布的话题'''
        yield event.plain_result("🕙 正在拉取大批量最新发布...")
        ...
        try:
            async with AsyncSession(impersonate="chrome120") as s:
                while len(all_topics) < limit:
                    ...
                    page += 1
                    await asyncio.sleep(0.3) # 稍微加快请求速度
    ...
    def _create_forward_nodes(self, event, items, title_prefix):
        ...
        # 每 60 条话题分一个节点，确保单节点字数在 QQ 安全范围内
        chunk_size = 60 
        for i in range(0, len(items), chunk_size):
            ...
            nodes.append(Node(uin=uin, name="LINUX DO 助手", content=[Plain(full_text.strip())]))
